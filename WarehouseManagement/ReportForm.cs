...
namespace WarehouseManagement
{
    public partial class ReportForm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string path = "";
            if (!checkedListBox1.GetItemChecked(3) && !checkedListBox1.GetItemChecked(2) && !checkedListBox1.GetItemChecked(1) && !checkedListBox1.GetItemChecked(0))
            {
                MessageBox.Show("You must choose at least one option");
            }
            else
            {
                if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    if ((myStream = saveFileDialog1.OpenFile()) != null && !string.IsNullOrWhiteSpace(saveFileDialog1.FileName))
                    {
